...
`on delete cascade` の連鎖挙動 (運用上の注意):

- `auth.users` が削除されると `editors` の行も連鎖削除される。Supabase ダッシュボードでユーザーを削除した直後に再招待しようとしても許可リストから消えているため `handle_new_user` で拒否される。
- 編集者の所有権を保ったまま auth セッションだけ無効化したい (再招待時にロール / display_name を保持したい) ケースが要件として出たら、この FK を `on delete set null` に変更し、`editors.user_id` を nullable のまま運用する別 migration を切る。Phase 1 では `cascade` のまま進めて問題ない (個人ブログで運用者が自分のみ、削除も自分の判断で行うため)。
- `editors` 自体が削除されると `posts.author_id (on delete restrict)` で参照中の記事があれば失敗する。記事を残すには `posts.author_id` を別 editor に付け替える先行 UPDATE が必要 (Phase 1 では管理画面 UI に「記事の移管」を作るかは要相談)。
