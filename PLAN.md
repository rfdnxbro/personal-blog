...
- [x] **0-2** ツールチェーンスキャフォールド (Next.js 16 / React 19 / TypeScript 6 / Tailwind v4 / pnpm 11 / Node 22 / `.env.example`)
...
- `.env.example` (`NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` / `SUPABASE_SECRET_KEY` / `ADMIN_EMAIL`)
...
- Hono は **薄いレイヤ** に徹する:
  ...
  - エラー → HTTP ステータスマッピング
- Hono は **必ず `createServerClient` の authenticated client 経由で Supabase を叩く**。`SUPABASE_SECRET_KEY` (Supabase 新仕様の `sb_secret_***`、Legacy の service_role JWT を置き換える) を使う `createClient` は以下に限定:
...
- `SUPABASE_SECRET_KEY` への参照は文字列リテラル grep で検出可能な単一経路に揃える
...
- 初期 admin メール (`ADMIN_EMAIL`) を `.env` / Vercel env (production + preview) / GitHub Actions secrets に登録
